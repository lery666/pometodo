...
>
> 当前源码版本号为 **0.1.3**。开发中的修改与官网下载的安装包可能不同；版本号相同不代表文件完全相同。已知问题见 [KNOWN_ISSUES.md](KNOWN_ISSUES.md)。

## 使用方式

普通用户可从 [石榴官网](https://www.shiliux.com/) 的 PomeTodo 入口获取安装包，不需要配置开发环境。
...
- 官方 AI 通过微信扫码登录使用。当前产品规则为首次一次性 50 次成功整理、30 天有效；25 元购买 500 次成功整理，付费次数不设到期日。仅成功整理扣次数。实际服务状态以官方客户端和购买页面为准。
- 源码公开不等于官方云端服务免费或无限使用。PomeTodo 可以独立于 PomeType 使用。
